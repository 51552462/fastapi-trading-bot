...
STOP_CONFIRM_N    = int(os.getenv("STOP_CONFIRM_N", "3"))         # ← 추가: 연속 N회 확인 시에만 컷
...
_STOP_CNT: Dict[str, int] = {}     # ← 추가: 연속 위반 카운터
...
def _mark_stop_fired(symbol: str, side: str):
    with _STOP_LOCK:
        ...
        _STOP_CNT[_key(symbol, side)] = 0  # 컷 후 카운터 리셋
    ...
    while True:
        try:
            for p in get_open_positions():
                ...
                move = ((entry-last)/entry if side=="long" else (last-entry)/entry)  # 불리 이동 비율(+)
                hit = bool(move >= STOP_PRICE_MOVE)

                cnt = _bump_stop_cnt(symbol, side, hit)
                if hit and cnt >= max(1, STOP_CONFIRM_N):          # ← N회 연속일 때만 컷
                    if not _stop_recently_fired(symbol, side):
                        _mark_stop_fired(symbol, side)
                        send_telegram(f"⛔ {symbol} {side.upper()} emergencyStop ≥{STOP_PRICE_MOVE*100:.2f}% (x{cnt})")
