...
# 비상컷(연속확인) – 1~4H 휩쏘 완화 기본값
STOP_PRICE_MOVE   = float(os.getenv("STOP_PRICE_MOVE", "0.02"))   # 0.02 = 2%
STOP_CHECK_SEC    = float(os.getenv("STOP_CHECK_SEC", "2.0"))
STOP_COOLDOWN_SEC = float(os.getenv("STOP_COOLDOWN_SEC", "60.0"))
...
def _update_capacity():
    with _CAP_LOCK:
        ...
        _CAPACITY["short_blocked"] = blocked  # 필요시 분리 가능
        ...
        try:
            for p in get_open_positions():
                ...
                if not symbol or side not in ("long", "short") or entry <= 0:
                    continue
                _ = get_last_price(symbol)  # 호출 유지(확장 포인트)
        ...
        try:
            ...
            for pkey, item in entry_items:
                ...
                if _local_has_any(sym):
                    _mark_done("entry", pkey, "(local_exists)")
                    continue
                ...
                with _lock_for(key):
                    try:
                        ...
                        if now - item.get("last_try", 0.0) < RECON_INTERVAL_SEC - 1:
                            continue
                        _set_busy(key)
                        tgt = float(item.get("size") or 0.0) or 0.0
                        resp = place_reduce_by_size(sym, tgt, side)
                        item["last_try"] = now
                        item["attempts"] = item.get("attempts", 0) + 1
                        if str(resp.get("code", "")) == "00000":
