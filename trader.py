...
def capacity_status():
    with _CAP_LOCK:
        return {"blocked": _CAPACITY["blocked"], "last_count": _CAPACITY["last_count"],
                "max": MAX_OPEN_POSITIONS, "interval": CAP_CHECK_SEC, "ts": _CAPACITY["ts"]}

def can_enter_now() -> bool:
    with _CAP_LOCK: return not _CAPACITY["blocked"]
...
def _capacity_loop():
    ...
    while True:
        try:
            ...
            now = time.time(); blocked = count >= MAX_OPEN_POSITIONS
            with _CAP_LOCK:
                _CAPACITY.update({"blocked": blocked, "last_count": count, "ts": now})
            if prev_blocked is None or prev_blocked != blocked:
                state = "BLOCKED (>= cap)" if blocked else "UNBLOCKED (< cap)"
                try: send_telegram(f"ℹ️ Position capacity {state} | {count}/{MAX_OPEN_POSITIONS}")
                except: pass
...
def _strict_try_reserve() -> bool:
    try: open_now_remote = len(get_open_positions())
    except: open_now_remote = 0
    ...
    with _RES_LOCK, _CAP_LOCK:
        ...
        _RESERVE["count"] += 1; return True
def _strict_release():
    with _RES_LOCK:
        if _RESERVE["count"] > 0: _RESERVE["count"] -= 1
    ...
    with _lock_for(key):
        ...
        if abs(float(pct) - TP3_PCT) <= 1e-6:
            with _PENDING_LOCK:
                ...
                _PENDING["tp"][pk] = {"symbol": symbol, "side": side, "stage": 3, "pct": float(pct),
...
def close_position(symbol: str, side: str = "long", reason: str = "manual"):
    ...
    with _lock_for(key):
        ...
        if success or ok:
            ...
            # 엔트리 중복 방지(닫힌 직후 잠깐 recent_ok) — 필요 없으면 주석
            _mark_recent_ok(key)
...
def reduce_by_contracts(symbol: str, contracts: float, side: str = "long"):
    ...
    with _lock_for(key):
        ...
        if qty <= 0: send_telegram(f"⚠️ reduceByContracts 스킵: step 미달 {key}"); return
        resp = place_reduce_by_size(symbol, qty, side)
        if str(resp.get("code", "")) == "00000": send_telegram(f"🔻 Reduce {qty} {side.upper()} {symbol}")
        else: send_telegram(f"❌ Reduce 실패 {key} → {resp}")
