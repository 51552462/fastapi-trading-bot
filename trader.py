...
_CAPACITY = {
    "blocked": False,         # 하위호환
    "last_count": 0,          # 하위호환(전체)
    "short_blocked": False,   # 숏 기준
    "short_count": 0,         # 숏 기준
    "ts": 0.0
}
...
def _local_short_count() -> int:
    with _POS_LOCK:
        return sum(1 for k in position_data.keys() if k.endswith("_short"))
...
# ── capacity guard (숏만 cap) ─────────────────────────────────
def _count_remote_shorts() -> int:
    try:
        return sum(1 for p in get_open_positions() if (p.get("side") or "").lower() == "short")
    except Exception:
        return 0
...
def _capacity_loop():
    prev_short_blocked = None
    while True:
        try:
            remote_shorts = _count_remote_shorts()
            local_shorts  = _local_short_count()
            short_count   = remote_shorts + local_shorts

            total_count = len(get_open_positions()) + _local_open_count()  # 하위호환

            now = time.time()
            short_blocked = short_count >= MAX_OPEN_POSITIONS

            with _CAP_LOCK:
                _CAPACITY["short_blocked"] = short_blocked
                _CAPACITY["short_count"]   = short_count
                _CAPACITY["last_count"]    = total_count
                _CAPACITY["blocked"]       = short_blocked  # 하위호환: 숏 기준
                _CAPACITY["ts"]            = now

            if prev_short_blocked is None or prev_short_blocked != short_blocked:
                state = "BLOCKED (short>=cap)" if short_blocked else "UNBLOCKED (short<cap)"
                try:
                    send_telegram(f"ℹ️ Short capacity {state} | {short_count}/{MAX_OPEN_POSITIONS}")
                except Exception:
                    pass
                prev_short_blocked = short_blocked
...
def _strict_try_reserve(side: str) -> bool:
    ...
    local_shorts = _local_short_count()
    with _RES_LOCK:
        effective = remote_shorts + local_shorts + _RESERVE["short"]
...
# ── 중복 진입 가드: inflight & recent-success ─────────────────
...
def enter_position(symbol: str, usdt_amount: float, side: str = "long", leverage: float = None):
    ...
    if not _strict_try_reserve(side):
        st = capacity_status()
        send_telegram(f"⏸️ retry_hold STRICT {symbol} {side} {st['short_count']}/{MAX_OPEN_POSITIONS}")
        return
    try:
        # periodic capacity (숏만 적용)
        if not can_enter_now(side):
            st = capacity_status()
            send_telegram(f"⏳ ENTRY HOLD (periodic) {symbol} {side} {st['short_count']}/{MAX_OPEN_POSITIONS}")
            return
...
BE_AFTER_STAGE   = int(os.getenv("BE_AFTER_STAGE", "1"))       # 1=TP1부터
        ...
        try:
            ...
            for pkey, item in entry_items:
                ...
                if not _strict_try_reserve(side):
                    if TRACE_LOG:
                        st = capacity_status()
                        send_telegram(f"⏸️ retry_hold STRICT {sym} {side} {st['short_count']}/{MAX_OPEN_POSITIONS}")
                    continue
