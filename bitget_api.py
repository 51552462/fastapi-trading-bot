# -*- coding: utf-8 -*-
"""
Bitget REST adapter (ONE-WAY mode)
- Provides stable functions for main/trader
"""
...
def convert_symbol(s: str) -> str:
    """
    Normalize various inputs to our 'core' form (e.g., 'BTCUSDT').
    - Accepts 'BTCUSDT_UMCBL', 'BTCUSDT-PERP', 'BINANCE:BTCUSDT.P', etc.
    """
    if not s: return ""
    t = str(s).upper().strip()
    # split exchange prefix if present
    if ":" in t:
        t = t.split(":")[-1]
    # remove separators
    for sep in ["_", "-", "."]:
        t = t.replace(sep, "")
    # remove contract suffixes
    t = t.replace("UMCBL", "").replace("PERP", "")
    # keep USDT/USDC pairs (we use USDT by default)
    return t
...
def _http(method: str, path: str, query: Optional[Dict[str,Any]]=None, body: Optional[Dict[str,Any]]=None) -> Dict[str,Any]:
    ...
    ts = _ts_ms()
    sign = _sign(ts, method, path, query, body)
...
def _req_public(method, path, params=None):
    return _http(method, path, params, None)

def _req_private(method, path, body=None, query=None):
    return _http(method, path, query, body)
...
def set_leverage(core: str, leverage: float) -> Dict[str,Any]:
    sym = _core_to_umcbl(core)
    body = {"symbol": sym, "marginCoin": "USDT", "leverage": str(int(leverage)), "holdSide":"long"}
    j = _req_private("POST", "/api/mix/v1/account/setLeverage", body)
    # set both sides in oneway just in case
    body["holdSide"] = "short"
    _ = _req_private("POST", "/api/mix/v1/account/setLeverage", body)
    return j
...
def _compute_size(core: str, notional_usdt: float, leverage: float) -> float:
    ...
    size_step = get_symbol_spec(core)["sizeStep"]
    return round_down_step(size, size_step)
...
def place_reduce_by_size(core: str, size: float, side: str) -> Dict[str,Any]:
    ...
    if size <= 0: 
        return {"code":"LOCAL_BAD_SIZE","msg":"size<=0"}
    body = {
        "symbol": sym,
        "marginCoin": "USDT",
        "productType": "USDT-FUTURES",
        "size": f"{size}",
        "side": "buy" if (side or "").lower()=="short" else "sell",  # inverse for reduce
        "orderType": "market",
        "force": "gtc",
        "reduceOnly": True,
    }
...
def close_all_for_symbol(core: str) -> Dict[str,Any]:
    sym = _core_to_umcbl(core)
    body = {"symbol": sym, "marginCoin":"USDT", "productType":"USDT-FUTURES"}
