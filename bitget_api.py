...
def convert_symbol(s: str) -> str:
    """
    다양한 표기(BTCUSDT, BTC-USDT, BTCUSDT_UMCBL, BINANCE:BTCUSDT, BTCUSDT.PERP 등)를
    내부 표준 'BTCUSDT'로 정규화.
    """
    if not s:
        return ""
            ...
            if (it.get("symbol") or "").upper()==sym:
                ...
                if ss:
                    try:
                        ...
                        size_step = 10 ** (-int(ss))  # volumePlace가 자리수인 경우
                return {"sizeStep": size_step, "priceStep": price_step}
    except Exception:
        pass
    return {"sizeStep": size_step, "priceStep": price_step}
...
def set_leverage(core: str, leverage: float) -> Dict[str, Any]:
    """
    v2 set-leverage (oneway에서는 단일 레버리지)
    """
    sym = _core_to_umcbl(core)
    body = {
        "symbol": sym,
...
def _normalize_side_for_oneway(side: str) -> str:
    """
    ONEWAY에서 진입/감축 모두 buy/sell로 보냄(reduceOnly로 감축 구분)
    """
    s = (side or "").lower()
    if s == "long":
        return "buy"
