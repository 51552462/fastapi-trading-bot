...
정리
- V2 엔드포인트(마켓/주문/계정/포지션)는 기본 심볼(ex: BTCUSDT) + productType 사용
- V1 폴백 엔드포인트만 거래소 심볼(ex: BTCUSDT_UMCBL) 사용
- 가격 조회: v2 단건(+/-productType) → v2 목록(+/-productType) → v2 캔들 → v1 티커 → v1 호가(mid)
- 원웨이, 마진모드(crossed/isolated), 레버리지, 부분청산/전량청산 모두 포함
"""
...
# amount 해석: notional(USDT) / margin(증거금×레버리지)
...
# 심볼 캐시: core(BTCUSDT) -> (exchange_symbol, ts)
...
# ---------------- Contract map (for v1 fallback only) ----------------
def _load_symbol_map() -> Dict[str, str]:
    ...
    try:
        for it in j.get("data") or []:
            ex_sym = (it.get("symbol") or "").upper()           # ex) BTCUSDT_UMCBL
            core   = convert_symbol(ex_sym)                      # ex) BTCUSDT
            if core:
                out[core] = ex_sym
    except Exception:
        pass
...
def get_last_price(core: str) -> Optional[float]:
    """
    Ultra-robust price retrieval (V2 base 심볼 중심, V1 폴백):
    ...
    """
    base = _base_symbol(core)                       # ex) DOGEUSDT
    ex_v1 = _resolve_exchange_symbol_for_v1(core)   # ex) DOGEUSDT_UMCBL
...
# ---------------- Account / Positions (V2 base 심볼 사용) ----------------
...
def place_reduce_by_size(core: str, contracts: float, side: str) -> Dict[str, Any]:
    """contracts 만큼 시장가 reduceOnly (분할청산용). side=현재포지션방향(long/short)"""
    base = _base_symbol(core)
