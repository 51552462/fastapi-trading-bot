...
def _req_private(method: str, path: str, body: Optional[dict] = None, query: Optional[dict] = None) -> dict:
    ts = _ts_ms()
    qstr = ""
    if query:
    ...
    try:
        for it in j.get("data") or []:
            if convert_symbol(it.get("symbol") or "") == base:
                if it.get("priceTick"):
                    price_step = float(it["priceTick"])
                if it.get("sizeTick"):
                    size_step = float(it["sizeTick"])
...
def get_last_price(core: str) -> Optional[float]:
    ...
    for params in (
        ...
        j = _req_public("GET", params[0], params[1])
        try:
            d = j.get("data") or {}
...
def set_leverage(core: str, lev: float) -> Dict[str, Any]:
    return _req_private(
        ...
        {
            "symbol": convert_symbol(core),
            "productType": PRODUCT_TYPE,
            "marginCoin": "USDT",
            "leverage": str(int(lev or 1)),
            "holdSide": "long",
            "marginMode": ("crossed" if _margin_mode() == "crossed" else "isolated"),
        },
    )
...
def get_open_positions(symbol: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    def _parse_v2(j: dict) -> List[Dict[str, Any]]:
    ...
    for q in v2_queries:
        try:
            ...
            res = _parse_v2(j)
            if BITGET_DEBUG:
                _dbg("v2 all-position", q, "=>", len(res))
            if res:
                out = res
                break
        except Exception as e:
...
def place_market_order(core: str, amount_usdt: float, side: str, leverage: float) -> Dict[str, Any]:
    ...
    body = {
        ...
        "marginCoin": "USDT",
        "size": f"{size}",
        "side": _normalize_side_for_oneway(side),
        "orderType": "market",
